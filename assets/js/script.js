...
onkeydown = function (evt) {
    ...
    if (!itemDetails) {
        ...
        console.log("itemDetails = " + evt.keyCode);
        switch (evt.keyCode) {
            case 13:
                console.log("img FS");
                afficheImageFS();
...
// var test = document.getElementById("video1").getElementsByTagName("video")[0]
// test.muted = true;
// test.autoplay = true;
// test.loop = true;
...
function afficheCategory(pages) {
    for (let i = 0; i < pages.length; i++) {
        ...
        for (let j = 0; j < categoryItem.length; j++) {
            ...
            var newDiv = document.createElement('div');
            console.log(categoryItem[j].image, categoryItem[j].image.includes("http"));
            var url = `https://hospitality.ansetech.com/host/files/images/pages/${categoryItem[j].image}`;
            if (categoryItem[j].image.includes("http"))
                url = categoryItem[j].image;
